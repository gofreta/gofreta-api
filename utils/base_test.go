...
func TestRenderTemplateFiles(t *testing.T) {
	...
	for _, scenario := range testScenarios {
		...
		result, resultErr := RenderTemplateFiles(scenario.Data, templateFiles...)
...
func TestRenderTemplateStrings(t *testing.T) {
	testScenarios := []struct {
		Data      interface{}
		Templates []string
		Expected  string
	...
	for _, scenario := range testScenarios {
		result, resultErr := RenderTemplateStrings(scenario.Data, scenario.Templates...)
		if result != scenario.Expected {
			t.Errorf("Expected %s, got %s (error: %v)", scenario.Expected, result, resultErr)
		}
	}
