...
func TestKeyDAO_ensureIndexes(t *testing.T) {
	...
	form := &models.KeyForm{Model: model, Title: "test", Access: map[string][]string{"test": []string{"index", "view"}}}
...
func TestKeyDAO_GetOne(t *testing.T) {
	...
	testScenarios := []struct {
		...
		{nil, false, "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjAsImlkIjoiNWE3NWVlNjNlMTM4MjMzNjcyOGMyYWRkIiwibW9kZWwiOiJrZXkifQ.azKosrX5iFaERlJHrIGSXeHBvtrUiViMzItjAXGYmcs"},
		{bson.M{"token": "missing"}, true, ""},
		{
			bson.M{"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjAsImlkIjoiNWE4YTk4Y2RlMTM4MjMwZWNkOTE1ZDM1IiwibW9kZWwiOiJrZXkifQ.euqQzO2sou8IOrhMML2Y2McbSQyVKBaZG7mdt2OdEYE"},
			false,
...
func TestKeyDAO_Create(t *testing.T) {
	...
	testScenarios := []struct {
		...
		{"", nil, true},
		{"Test", nil, true},
		{"", map[string][]string{"test": []string{"index", "view"}}, true},
		{"Test", map[string][]string{"test": []string{"index", "view"}}, false},
	}
