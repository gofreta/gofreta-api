...
func GetPaginationSettings(c *routing.Context, total int) (int, int) {
	...
	if settings.Limit <= 0 {
		settings.Limit = gofreta.App.Config.GetInt("pagination.defaultLimit")
	} else if maxLimit := gofreta.App.Config.GetInt("pagination.maxLimit"); settings.Limit > maxLimit {
