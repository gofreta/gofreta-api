...
func TestMediaApi_upload(t *testing.T) {
	...
	// reset
	defer gofreta.App.Config.Set("upload.maxSize", gofreta.App.Config.GetFloat64("upload.maxSize"))
	...
	testScenarios := []struct {
		...
		// check file size constraint
		{
			...
			&TestApiScenario{
				Data:            `GIF87a`,
				ExpectedCode:    200,
				ExpectedContent: []string{`{"items":[],"errors":["Media is too big."]}`},
			},
		},
		// single invalid file
		{
			...
			&TestApiScenario{
				Data:            `invalid`,
				ExpectedCode:    200,
				ExpectedContent: []string{`"items":[]`, `"errors":["Invalid or unsupported file type."]`},
			},
		},
		// valid and invalid file
		{
			...
			&TestApiScenario{
				Data:            `GIF87a,invalid`,
				ExpectedCode:    200,
				ExpectedContent: []string{`"errors":["Invalid or unsupported file type."]`, `{"items":[{"id":"`, `"type":"image"`, `"title":"test0"`, `"path":"`},
			},
		},
	...
	for _, item := range testScenarios {
		gofreta.App.Config.Set("upload.maxSize", item.MaxSize)
...
func TestUploadFile(t *testing.T) {
	...
	for _, scenario := range testScenarios {
		gofreta.App.Config.Set("upload.maxSize", scenario.MaxSize)
