...
func TestMediaDAO_Delete(t *testing.T) {
	...
	// create temp file
	tmp, _ := ioutil.TempFile("", "test_delete")
	tmp.Write([]byte("test"))
	tmp.Close()
...
func TestToAbsMediaPath(t *testing.T) {
	baseUrl := gofreta.App.Config.GetString("baseUrl")
	...
	if !strings.HasPrefix(model.Path, baseUrl) {
		t.Errorf("Expected path to start with %s, got %s", baseUrl, model.Path)
	}
}
...
func TestToAbsMediaPaths(t *testing.T) {
	...
	for _, item := range items {
		if !strings.HasPrefix(item.Path, baseUrl) {
			t.Errorf("Expected path to start with %s, got %s", baseUrl, item.Path)
		}
	}
}
