...
func (m User) NewAuthToken(exp int64) (string, error) {
	...
	signingKey := gofreta.App.Config.GetString("jwt.signingKey")
...
func (m User) HasValidResetPasswordHash() bool {
	...
	if len(parts) == 2 {
		if castedVal, castErr := strconv.Atoi(parts[1]); castErr == nil {
			...
			secret := gofreta.App.Config.GetString("resetPassword.secret")
...
func (m UserUpdateForm) Validate() error {
	return validation.ValidateStruct(&m,
		...
		validation.Field(&m.Status, validation.Required, validation.In(UserStatusActive, UserStatusInactive)),
		validation.Field(&m.Access, validation.Required),
		validation.Field(&m.OldPassword, validation.By(checkOldPassword(m.Model))),
		validation.Field(&m.Password, validation.By(checkOptionalRequirement(m.OldPassword))),
		validation.Field(&m.PasswordConfirm, validation.By(checkPasswordConfirm(m.Password))),
	)
}
...
func (m UserUpdateForm) ResolveModel() *User {
	...
	if m.OldPassword != "" &&
		model.ValidatePassword(m.OldPassword) &&
		m.Password != "" &&
		m.Password == m.PasswordConfirm {
...
// checkOldPassword validates and checks whether the validating nonempty password string is correct for the provided user model.
func checkOldPassword(user *User) validation.RuleFunc {
	return func(value interface{}) error {
		v, _ := value.(string)
